...
import datetime    # Python's internal date 

def datediff(d1, d2):
    """Calculates the difference between two dates in days (day, month, year)
    Args:
...
def timediff(t1, t2):
    ...
    # Funktion calcultates a time delta wihch supports only seconds or microseconds
...
def datediff2(d1, d2, unit):
    """Returns  difference between 2 dates in chosen unit
    ...
    Returns:
        float: difference between 2 dates in chosen unit (day or year) 
    """
    d1 = datetime.datetime.strptime(d1, "%Y-%m-%d")
    d2 = datetime.datetime.strptime(d2, "%Y-%m-%d")
    difference = abs((d2 - d1).days) # Timedelta in days
    units = {'day': 1, 'year': 365} # Dictionary for unit dividers
    divider = units[unit] # Choose by unit argument
    value = difference / divider  
    return value

def timediff2(t1, t2, unit ):
    """Calculates the difference between two time values in chosen units (day, minutes or year)
    Args:
        t1 (str): time value in format hh:mm:ss
        t2 (str): time value in format hh:mm:ss
    Returns:
